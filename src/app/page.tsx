...
export default function MarketingPage() {
  ...
  // Video Ad Player State (180 seconds / 3 minutes)
  ...
  useEffect(() => {
    if (!showVideoModal || !voiceoverEnabled) {
    ...
    if (videoCurrentTime === 0 && isVideoPlaying) {
      speak("Are endless calculations and manual report card piles slowing down your school's progress? Compiling marks and manual grading can take weeks, leading to errors and delays.");
    } else if (videoCurrentTime === 20) {
      speak("Introducing SchoolPro Uganda! The ultimate management software designed for primary and secondary schools. Register, enroll students, and assign teachers in minutes. Everything is safely backed up in the cloud.");
    } else if (videoCurrentTime === 45) {
      speak("For primary levels, SchoolPro simplifies UNEB grading. Enter raw scores, and the system instantly maps grades from distinction one to failure nine, computes aggregates, and automatically assigns divisions one to four.");
    } else if (videoCurrentTime === 75) {
      speak("Fully aligned with Uganda's Lower Secondary CBC! Grade competency levels, projects, and end-of-term exams effortlessly. The software auto-calculates continuous assessments and generates achievement descriptors.");
    } else if (videoCurrentTime === 105) {
      speak("Say goodbye to cashbook confusion. SchoolPro tracks student tuition fees, invoices, payments, and outstanding balances. School directors can view real-time collections and financial dashboards instantly.");
    } else if (videoCurrentTime === 135) {
      speak("Engage parents like never before. With integrated SMS alerts, you can broadcast academic results and fee reminders directly to parents' mobile phones. Parents can also log in to monitor their child's performance.");
    } else if (videoCurrentTime === 165) {
      speak("Join hundreds of schools countrywide modernizing their systems. Register for free today at portal dot laptertech dot store, or call zero seven six three, eight two one, zero four two. SchoolPro Uganda: School management made simple.");
    }
  }, [videoCurrentTime, showVideoModal, voiceoverEnabled, isVideoPlaying]);
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#ffffff", color: "#1e293b", fontFamily: "var(--font-sans)" }} className="animate-fade-in">
            ...
            <div className="animate-slide-up">
              ...
              <div style={{ textAlign: "center", marginBottom: "60px" }}>
                ...
                <div className="flex justify-center gap-2 flex-mobile-col align-center flex-wrap" style={{ marginTop: "24px" }}>
                  ...
                  <button onClick={() => { setShowVideoModal(true); setIsVideoPlaying(true); setVideoCurrentTime(0); }} className="btn btn-outline hover-scale" style={{ padding: "14px 32px", color: "var(--primary)", borderColor: "var(--primary)", background: "var(--primary-light)", fontSize: "15px", width: "100%", maxWidth: "250px", justifyContent: "center" }}>
                    🎥 Play 3m Video Ad
                  </button>
                </div>
      ...
      {/* 3-Minute Video Ad Player Modal */}
      {showVideoModal && (
        <div style={{
          position: "fixed",
          top: 0,
          ...
          {/* Close Button */}
          <button 
            onClick={() => { setShowVideoModal(false); setIsVideoPlaying(false); window.speechSynthesis?.cancel(); }}
            style={{
              position: "absolute",
              top: "20px",
              right: "20px",
          ...
          <div style={{
            ...
            <div style={{
              ...
              <span>SCHOOLPRO ADVERT PREVIEW</span>
              <span style={{ color: "var(--secondary)" }}>
                {videoCurrentTime < 20 && "Scene 1: Stressed Administration (The Pain)"}
                {videoCurrentTime >= 20 && videoCurrentTime < 45 && "Scene 2: Introducing SchoolPro & Cloud Dashboard"}
                {videoCurrentTime >= 45 && videoCurrentTime < 75 && "Scene 3: Primary Level PLE & Auto-Grading"}
                {videoCurrentTime >= 75 && videoCurrentTime < 105 && "Scene 4: Lower Secondary CBC Mappings"}
                {videoCurrentTime >= 105 && videoCurrentTime < 135 && "Scene 5: Tuition Fees & Financial Tracking"}
                {videoCurrentTime >= 135 && videoCurrentTime < 165 && "Scene 6: Parent Engagement & SMS Alerts"}
                {videoCurrentTime >= 165 && "Scene 7: Registration & Support Outro"}
              </span>
            </div>

            {/* Video Active Screen (16:9) */}
            <div style={{
              width: "100%",
              aspectRatio: "16/9",
              position: "relative",
              ...
              {videoCurrentTime < 20 && (
                <div style={{
                  ...
                  <div style={{ fontSize: "60px", marginBottom: "16px", animation: "breathe 1.5s infinite" }}>📚📝❌</div>
                  <h3 style={{ color: "#f8fafc", fontSize: "26px", fontWeight: 800, lineHeight: 1.3, maxWidth: "550px" }}>
                    "Is your school administration drowning in piles of report cards and calculations?"
                  </h3>
                  <p style={{ color: "#ef4444", fontSize: "14px", marginTop: "16px", fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase" }}>
                    ⚠️ Mistakes in grading • Delayed results • Administrative overload
                  </p>
                </div>
              )}

              {/* Scene 2: Intro Brand Logo & Cloud Dashboard (20 - 45s) */}
              {videoCurrentTime >= 20 && videoCurrentTime < 45 && (
                <div style={{
                  display: "flex",
                  flexDirection: "column",
                  alignItems: "center",
                  ...
                  <img 
                    src="/logo-full.png" 
                    alt="SchoolPro Uganda Logo" 
                    style={{ height: "80px", width: "auto", objectFit: "contain", filter: "drop-shadow(0px 10px 20px rgba(0,0,0,0.5))" }} 
                  />
                  <h4 style={{ color: "#f8fafc", fontSize: "22px", marginTop: "24px", fontWeight: 700 }}>
                    Modern, Fast & Fully Secure Cloud Portal
                  </h4>
                  <p style={{ color: "#94a3b8", fontSize: "14px", marginTop: "10px", maxWidth: "550px" }}>
                    Set up your school, classes, teachers, and student directories in under 5 minutes. Accessible on both desktop and mobile devices securely.
                  </p>
                </div>
              )}

              {/* Scene 3: Primary Level PLE & Auto-Grading (45 - 75s) */}
              {videoCurrentTime >= 45 && videoCurrentTime < 75 && (
                <div style={{
                  width: "100%",
                  height: "100%",
                  padding: "30px",
                  ...
                  <div style={{
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", borderBottom: "1px solid rgba(255,255,255,0.1)", paddingBottom: "10px", marginBottom: "10px" }}>
                      <span style={{ color: "#f8fafc", fontWeight: 700 }}>Namubiru Agnes (Primary 7)</span>
                      <span style={{ color: "var(--secondary)", fontWeight: 700 }}>UNEB PLE Report</span>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr auto auto", gap: "8px", fontSize: "14px", color: "#cbd5e1" }}>
                      <span>Mathematics</span><span style={{ fontWeight: 600 }}>86%</span><span style={{ color: "#22c55e", fontWeight: 700 }}>D1</span>
                      <span>English</span><span style={{ fontWeight: 600 }}>78%</span><span style={{ color: "#22c55e", fontWeight: 700 }}>D2</span>
                      <span>Social Studies</span><span style={{ fontWeight: 600 }}>82%</span><span style={{ color: "#22c55e", fontWeight: 700 }}>D1</span>
                      <span>Basic Science</span><span style={{ fontWeight: 600 }}>90%</span><span style={{ color: "#22c55e", fontWeight: 700 }}>D1</span>
                    </div>
                    <div style={{ marginTop: "12px", paddingTop: "8px", borderTop: "1px solid rgba(255,255,255,0.1)", display: "flex", justifyContent: "space-between", fontWeight: 800, fontSize: "15px" }}>
                      <span style={{ color: "#94a3b8" }}>TOTAL AGGREGATE:</span>
                      <span style={{ color: "var(--secondary)" }}>5 (Division 1) 🌟</span>
                    </div>
                  </div>
                </div>
              )}

              {/* Scene 4: Lower Secondary CBC Mappings (75 - 105s) */}
              {videoCurrentTime >= 75 && videoCurrentTime < 105 && (
                <div style={{
                  width: "100%",
                  height: "100%",
                  padding: "30px",
                  ...
                  <div style={{
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", borderBottom: "1px solid rgba(255,255,255,0.1)", paddingBottom: "10px", marginBottom: "10px" }}>
                      <span style={{ color: "#f8fafc", fontWeight: 700 }}>Lower Secondary CBC grading</span>
                      <span style={{ color: "#38bdf8", fontWeight: 700 }}>NLSC Aligned</span>
                    </div>
                    <div style={{ fontSize: "14px", color: "#cbd5e1" }}>
                      <div style={{ marginBottom: "6px" }}>📊 **Topic:** Constructing Geometric Models</div>
                      <div style={{ marginBottom: "6px" }}>📝 **Activity Score:** <span style={{ color: "var(--secondary)", fontWeight: 700 }}>3 (Outstanding Achievement)</span></div>
                      <div style={{ padding: "8px", background: "rgba(255, 255, 255, 0.03)", borderRadius: "8px", fontSize: "12px", borderLeft: "3px solid var(--secondary)", fontStyle: "italic", lineHeight: 1.4 }}>
                        "Demonstrates deep conceptual understanding and constructs highly accurate shapes independently."
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {/* Scene 5: Tuition Fees & Financial Tracking (105 - 135s) */}
              {videoCurrentTime >= 105 && videoCurrentTime < 135 && (
                <div style={{
                  width: "100%",
                  height: "100%",
                  padding: "30px",
                  ...
                  <div style={{
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", borderBottom: "1px solid rgba(255,255,255,0.1)", paddingBottom: "10px", marginBottom: "10px" }}>
                      <span style={{ color: "#f8fafc", fontWeight: 700 }}>Bursar Finance Dashboard</span>
                      <span style={{ color: "#22c55e", fontWeight: 700 }}>Cashflow Tracker</span>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: "10px", fontSize: "14px", color: "#cbd5e1" }}>
                      <span>Total Expected Tuition:</span><span style={{ fontWeight: 700 }}>45,000,000 UGX</span>
                      <span>Total Fees Collected:</span><span style={{ color: "#22c55e", fontWeight: 700 }}>32,500,000 UGX</span>
                      <span>Outstanding Balances:</span><span style={{ color: "#ef4444", fontWeight: 700 }}>12,500,000 UGX</span>
                    </div>
                    <div style={{ marginTop: "12px", padding: "6px 10px", background: "rgba(34, 197, 94, 0.1)", border: "1px solid rgba(34, 197, 94, 0.2)", borderRadius: "8px", fontSize: "12px", color: "#4ade80", textAlign: "center" }}>
                      ✓ Simulates instant payments via Mobile Money or Bank Ledger exports.
                    </div>
                  </div>
                </div>
              )}

              {/* Scene 6: Parent Engagement & SMS Alerts (135 - 165s) */}
              {videoCurrentTime >= 135 && videoCurrentTime < 165 && (
                <div style={{
                  width: "100%",
                  height: "100%",
                  display: "flex",
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      💬 **SCHOOLPRO-SMS**
                      <br/>
                      Dear Parent, the outstanding balance for Mugisha John is **150,000 UGX**. Please clear to avoid inconvenience.
                    </div>
                  </div>
                </div>
              )}

              {/* Scene 7: Registration & Support Outro (165 - 180s) */}
              {videoCurrentTime >= 165 && (
                <div style={{
                  textAlign: "center",
                  padding: "30px",
                  display: "flex",
                  ...
                  <img src="/logo-full.png" alt="SchoolPro Uganda" style={{ height: "60px", marginBottom: "15px" }} />
                  <h4 style={{ color: "#f8fafc", fontSize: "20px", fontWeight: 800, marginBottom: "8px" }}>
                  ...
                  <p style={{ color: "#94a3b8", fontSize: "14px", marginBottom: "16px" }}>
                    📞 **Phone: 0763821042** | ✉️ **Email: laptertechnoloies@gmail.com**
                  </p>
                  <button 
                    onClick={() => { setShowVideoModal(false); setIsVideoPlaying(false); setActiveTab("register"); window.speechSynthesis?.cancel(); }}
                    style={{
              ...
              <div style={{
                ...
                {videoCurrentTime < 20 && "Are endless calculations and manual report card piles slowing down your school's progress? Compiling marks and manual grading can take weeks, leading to errors and delays."}
                {videoCurrentTime >= 20 && videoCurrentTime < 45 && "Introducing SchoolPro Uganda! The ultimate management software designed for primary and secondary schools. Register, enroll students, and assign teachers in minutes. Everything is safely backed up in the cloud."}
                {videoCurrentTime >= 45 && videoCurrentTime < 75 && "For primary levels, SchoolPro simplifies UNEB grading. Enter raw scores, and the system instantly maps grades from distinction one to failure nine, computes aggregates, and automatically assigns divisions."}
                {videoCurrentTime >= 75 && videoCurrentTime < 105 && "Fully aligned with Uganda's Lower Secondary CBC! Grade competency levels, projects, and end-of-term exams effortlessly. The software auto-calculates continuous assessments and generates achievement descriptors."}
                {videoCurrentTime >= 105 && videoCurrentTime < 135 && "Say goodbye to cashbook confusion. SchoolPro tracks student tuition fees, invoices, payments, and outstanding balances. School directors can view real-time collections and financial dashboards instantly."}
                {videoCurrentTime >= 135 && videoCurrentTime < 165 && "Engage parents like never before. With integrated SMS alerts, you can broadcast academic results and fee reminders directly to parents' mobile phones. Parents can also log in to monitor their child's performance."}
                {videoCurrentTime >= 165 && "Join hundreds of schools countrywide modernizing their systems. Register for free today at portal.laptertech.store, or call 0763821042. SchoolPro Uganda: School management made simple."}
              </div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                <span style={{ color: "#94a3b8", fontSize: "12px", width: "45px", textAlign: "right" }}>
                  {Math.floor(videoCurrentTime / 60)}:{(videoCurrentTime % 60) < 10 ? `0${videoCurrentTime % 60}` : videoCurrentTime % 60}
                </span>
                <input 
                  type="range" 
                  min="0" 
                ...
                />
                <span style={{ color: "#94a3b8", fontSize: "12px", width: "45px" }}>3:00</span>
              </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
                  ...
                  <button 
                    onClick={() => {
                      ...
                      if (voiceoverEnabled) window.speechSynthesis?.cancel();
                    }}
                    style={{ 
                      background: "rgba(255, 255, 255, 0.05)", 
                      border: "1px solid rgba(255, 255, 255, 0.1)", 
                  ...
                  >
                    {voiceoverEnabled ? <Volume2 size={16} color="var(--secondary)" /> : <VolumeX size={16} color="#ef4444" />}
                    <span>{voiceoverEnabled ? "Voiceover Active (TTS)" : "Voiceover Muted"}</span>
                  </button>
                </div>
              </div>
            </div>
